...
geometry_msgs::Pose eigenTransformToPose(const Eigen::Matrix4d& T) {
...
bool MMPlannerManager::computeUrdfEeTransform(const Eigen::VectorXd &joints, Eigen::Matrix4d &T) const
{
  if (!urdf_fk_ready_ || !urdf_fk_solver_ || joints.size() < 6) return false;
  ...
  KDL::Frame frame;
  if (urdf_fk_solver_->JntToCart(q, frame) < 0) return false;
  T.setIdentity();
  for (int r = 0; r < 3; ++r)
  ...
  void MMPlannerManager::initPlanModules(ros::NodeHandle &nh, PlanningVisualization::Ptr vis)
  {
    ...
    frontend_path_nav_pub_ = nh.advertise<nav_msgs::Path>("kinoastar/path_nav", 1, true);
    // Keep the predicted path separate from the measured execution trace.
    ...
    urdf_fk_ready_ = false;
    std::string robot_xml;
    if (nh.getParam("/robot_description", robot_xml)) {
      urdf::Model model;
      KDL::Tree tree;
      if (model.initString(robot_xml) && kdl_parser::treeFromUrdfModel(model, tree) &&
          tree.getChain("base_link", "arm_gripper_link", urdf_chain_)) {
        urdf_fk_solver_.reset(new KDL::ChainFkSolverPos_recursive(urdf_chain_));
        urdf_fk_ready_ = true;
        ROS_WARN_STREAM("[Planner] using URDF FK chain base_link -> arm_gripper_link, joints="
                        << urdf_chain_.getNrOfJoints());
      } else {
        ROS_ERROR("[Planner] failed to build URDF FK chain base_link -> arm_gripper_link");
      }
    } else {
      ROS_ERROR("[Planner] /robot_description is unavailable; cannot build URDF FK");
    }
                                                    ...
                                                    const int continous_failures_count)
  {
    ...
    if (flag_first_call || flag_polyInit){
      ...
      ROS_INFO("[Planner] A* initialization status=%d, segments=%zu", status,
               initMJO_container.size());
      // std::cout << "computeInit 4\n";
      if(status == KinoAstar::NO_PATH || status == KinoAstar::START_COLLISION || status == KinoAstar::GOAL_COLLISION){
        ROS_WARN("[Planner] A* initialization rejected: status=%d", status);
        return false;
      }
      ...
      nav_msgs::Path base_path_nav;
      nav_msgs::Path ee_path_nav;
      base_path_nav.header.frame_id = "world";
      base_path_nav.header.stamp = ros::Time::now();
      ee_path_nav.header = base_path_nav.header;
      for(unsigned int i = 0; i < simple_path_container.size(); ++i){
        ...
        for(unsigned int j = 0; j < simple_path_container[i].size(); ++j){
          ...
          base_pose.header = base_path_nav.header;
          base_pose.pose.position.x = (simple_path_container[i])[j](0);
          base_pose.pose.position.y = (simple_path_container[i])[j](1);
          base_pose.pose.position.z = 0.0;
          base_pose.pose.orientation = yawToQuaternion((yaw_list_container[i])[j]);
          base_path_nav.poses.push_back(base_pose);

          Eigen::Vector3d car_state((simple_path_container[i])[j](0), (simple_path_container[i])[j](1), (yaw_list_container[i])[j]);
          Eigen::Matrix4d T_car;
          mm_config_->CarState2T(car_state, T_car);
          Eigen::Matrix4d T_now;
          if (urdf_fk_ready_ && computeUrdfEeTransform((simple_path_container[i])[j].tail(pp_.manipulator_dim_), T_now)) {
            T_now = T_car * T_now;
          } else {
            T_now = T_car * mm_config_->getTq0();
            std::vector<Eigen::Matrix4d> T_joint, T_joint_grad;
            mm_config_->getJointTrans((simple_path_container[i])[j].tail(pp_.manipulator_dim_), T_joint, T_joint_grad);
            for (int k = 0; k < pp_.manipulator_dim_; ++k) T_now = T_now * T_joint[k];
          }

          geometry_msgs::PoseStamped ee_pose;
          ee_pose.header = ee_path_nav.header;
          ee_pose.pose = eigenTransformToPose(T_now);
          ee_path_nav.poses.push_back(ee_pose);
        }
      ...
      // The A* path is only an initialization reference. The final path is
      // published below after optimization.
      // The initial A* path is only an initialization reference.  Do not
      // publish it as the executed EE path; the optimizer changes it.
    }
  ...
  bool MMPlannerManager::reboundReplan(
      ...
      const Eigen::VectorXd &display_start_pt, const double display_start_yaw)
  {
    static int count = 0;
    ...
    // Publish the EE path generated from the exact trajectory sent to the
    // controller, using the same CR10 gripper frame as the URDF.
    nav_msgs::Path executed_ee_path;
    nav_msgs::Path executed_base_path;
    executed_ee_path.header.frame_id = "world";
    // The controller evaluates the polynomial from this exact timestamp.
    // Using the publication time here shifts the orange reference relative
    // to the purple TF samples whenever optimization took non-zero time.
    executed_ee_path.header.stamp = ros::Time(traj_container_.singul_traj_data.start_time);
    executed_base_path.header = executed_ee_path.header;

    // The optimizer trajectory can be re-timed and its first sampled point
    // may be slightly inside the first polynomial piece.  Explicitly publish
    // the actual planning start state first, so RViz and the robot model have
    // a guaranteed continuous connection at the execution boundary.
    {
      // Replanning can take several seconds.  Read the measured state again
      // here; display_start_pt may already be stale by the time optimization
      // finishes, especially while a previous local trajectory is executing.
      Eigen::VectorXd measured_start = display_start_pt;
      double measured_yaw = display_start_yaw;
      ros::NodeHandle state_nh;
      auto measured_odom = ros::topic::waitForMessage<nav_msgs::Odometry>(
          "/mm/car/odom", state_nh, ros::Duration(0.2));
      auto measured_joints = ros::topic::waitForMessage<sensor_msgs::JointState>(
          "/mm/mani/joint_state", state_nh, ros::Duration(0.2));
      if (measured_odom) {
        measured_start(0) = measured_odom->pose.pose.position.x;
        measured_start(1) = measured_odom->pose.pose.position.y;
        measured_yaw = tf::getYaw(measured_odom->pose.pose.orientation);
      }
      if (measured_joints && measured_joints->position.size() >= static_cast<size_t>(pp_.manipulator_dim_)) {
        for (int i = 0; i < pp_.manipulator_dim_; ++i)
          measured_start(pp_.mobile_base_dim_ + i) = measured_joints->position[i];
      }

      Eigen::Vector3d start_car(measured_start(0), measured_start(1), measured_yaw);
      Eigen::Matrix4d T_start;
      mm_config_->CarState2T(start_car, T_start);
      T_start = T_start * mm_config_->getTq0();
      std::vector<Eigen::Matrix4d> start_joints, start_joint_grads;
      mm_config_->getJointTrans(measured_start.tail(pp_.manipulator_dim_), start_joints, start_joint_grads);
      for (const auto &joint : start_joints) T_start = T_start * joint;
      Eigen::Matrix4d T_gripper = Eigen::Matrix4d::Identity();
      T_gripper.block(0, 0, 3, 3) =
          (Eigen::AngleAxisd(-1.5707, Eigen::Vector3d::UnitY()) *
           Eigen::AngleAxisd(1.5707, Eigen::Vector3d::UnitZ())).toRotationMatrix();
      T_gripper(2, 3) = 0.0005;

      // Anchor the displayed planned path to the same TF chain used by RViz.
      // The internal FK state is still used by the optimizer, but its first
      // visualization point must not jump away from the actual robot.
      tf::StampedTransform start_tf;
      try {
        tf_listener_.waitForTransform("world", "arm_gripper_link", ros::Time(0),
                                      ros::Duration(0.2));
        tf_listener_.lookupTransform("world", "arm_gripper_link", ros::Time(0), start_tf);
        T_start.setIdentity();
        T_start.block<3, 3>(0, 0) = Eigen::Quaterniond(
            start_tf.getRotation().w(), start_tf.getRotation().x(),
            start_tf.getRotation().y(), start_tf.getRotation().z()).toRotationMatrix();
        T_start.block<3, 1>(0, 3) = Eigen::Vector3d(
            start_tf.getOrigin().x(), start_tf.getOrigin().y(), start_tf.getOrigin().z());
        T_gripper.setIdentity();
      } catch (const tf::TransformException &ex) {
        ROS_WARN("Could not anchor planned EE start to TF: %s", ex.what());
      }

      geometry_msgs::PoseStamped start_ee;
      start_ee.header = executed_ee_path.header;
      start_ee.pose = eigenTransformToPose(T_start * T_gripper);

      executed_ee_path.poses.push_back(start_ee);
    }

    // Use the exact local trajectory container serialized by
    // sendPolyTrajROSMsg(), so RViz displays the same trajectory version that
    // the controller executes.
    const double duration = traj_container_.singul_traj_data.duration;
    for (double t = 0.0; t <= duration; t += 0.02) {
        ...
        geometry_msgs::PoseStamped base_pose;
        base_pose.header = executed_base_path.header;
        base_pose.header.stamp = ros::Time(executed_ee_path.header.stamp.toSec() + t);
        base_pose.pose.position.x = state(0);
        base_pose.pose.position.y = state(1);
        base_pose.pose.position.z = 0.0;
        base_pose.pose.orientation = yawToQuaternion(car_state(2));
        executed_base_path.poses.push_back(base_pose);
        Eigen::Matrix4d T_now;
        mm_config_->CarState2T(car_state, T_now);
        Eigen::Matrix4d T_ee;
        if (urdf_fk_ready_ && computeUrdfEeTransform(state.tail(pp_.manipulator_dim_), T_ee)) {
          T_now = T_now * T_ee;
        } else {
          T_now = T_now * mm_config_->getTq0();
          std::vector<Eigen::Matrix4d> joints, joint_grads;
          mm_config_->getJointTrans(state.tail(pp_.manipulator_dim_), joints, joint_grads);
          for (const auto &joint : joints) T_now = T_now * joint;
          Eigen::Matrix4d T_gripper = Eigen::Matrix4d::Identity();
          T_gripper.block(0, 0, 3, 3) =
              (Eigen::AngleAxisd(-1.5707, Eigen::Vector3d::UnitY()) *
               Eigen::AngleAxisd(1.5707, Eigen::Vector3d::UnitZ())).toRotationMatrix();
          T_gripper(2, 3) = 0.0005;
          T_now = T_now * T_gripper;
        }
        executed_ee_path.poses.emplace_back();
        executed_ee_path.poses.back().header = executed_ee_path.header;
        executed_ee_path.poses.back().header.stamp = ros::Time(executed_ee_path.header.stamp.toSec() + t);
        executed_ee_path.poses.back().pose = eigenTransformToPose(T_now);
    }
