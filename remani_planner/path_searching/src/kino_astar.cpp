...
int KinoAstar::KinoAstarSearchAndGetSimplePath(const Eigen::VectorXd &start_pos, const Eigen::VectorXd &start_vel, const double start_yaw, const int start_singul, const bool start_gripper,
  ...
  ROS_INFO("[Timing] mobile-base KinoAstar search=%.3f ms, has_path=%s",
           (ros::WallTime::now() - planning_start).toSec() * 1000.0,
  ...
  if(has_path_){
    ...
    const ros::WallTime arm_search_start = ros::WallTime::now();
    ...
    ROS_INFO("[Timing] coupled arm search=%.3f ms, car_samples=%zu, dense_checks=%zu, success=%s",
             (ros::WallTime::now() - arm_search_start).toSec() * 1000.0,
             car_statelist.size(), car_statelist_check.size(), sample_succ ? "true" : "false");
  }else{
    std::vector<Eigen::Vector3d> car_statelist, car_statelist_check;
    std::vector<double> init_t_list;
    ...
    sample_succ = mani_sample_->sampleManiSearch(false,
        ...
        simple_path_container, singul_container,
        yaw_list_container, t_list_container);
    ROS_INFO("[Timing] arm search without car path=%.3f ms, car_samples=%zu, dense_checks=%zu, success=%s",
             (ros::WallTime::now() - arm_search_start).toSec() * 1000.0,
             car_statelist.size(), car_statelist_check.size(), sample_succ ? "true" : "false");
  }
  ...
  if(sample_succ){
    ...
    ROS_WARN("[KinoAstar] mobile-base path found/attempted, but coupled arm search returned NO_PATH");
    return KinoAstar::NO_PATH;
  }
}

void KinoAstar::visPath(std::vector<std::vector<Eigen::Vector3d>> path, bool pt){
  if (frontend_path_pub_.getNumSubscribers() == 0 && frontend_path_nav_pub_.getNumSubscribers() == 0){
    return;
  }
  ...
  frontend_path_nav_pub_.publish(nav_path);
}
  ...
  while (!open_set_.empty()){
    ...
    if((cur_node->state.head(2) - end_state_.head(2)).norm() < oneshot_range_ && initsearch){
      ...
      if(cur_node->parent != NULL){
        bool is_forward = Eigen::Vector2d(cos(cur_node->parent->state(2)), sin(cur_node->parent->state(2))).transpose() * (cur_node->state - cur_node->parent->state).head(2) > 0;
