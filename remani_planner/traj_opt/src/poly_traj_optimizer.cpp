...
namespace remani_planner
{
  ...
  bool PolyTrajOptimizer::OptimizeTrajectory_lbfgs(const std::vector<Eigen::MatrixXd> &iniState_container, 
    ...
    // Navigation goals keep the arm posture unchanged. The front-end has
    // already checked the dense base path against the URDF collision model;
    // avoid spending seconds on numerical collision-gradient optimization
    // when the valid minimum-snap seed can be used directly.
    bool stationary_arm = traj_num_ > 0;
    for (int i = 0; stationary_arm && i < traj_num_; ++i) {
      stationary_arm = (iniState_container_[i].col(0).tail(manipulator_dof_) -
                        finState_container_[i].col(0).tail(manipulator_dof_))
    ...
    if (stationary_arm) {
      ...
      for (int i = 0; i < traj_num_; ++i) {
        const int pieces = piece_num_container_[i];
        SnapOpt_container_[i].reset(iniState_container_[i], finState_container_[i], pieces);
    ...
    // Mesh/sphere clearance is evaluated for every control point.  Bound
    // the solve time so the FSM can replan instead of blocking indefinitely.
    // Bound the expensive URDF sphere finite-difference cost evaluations.
  ...
  bool PolyTrajOptimizer::IsNotFeasibie(const SingulTrajData &traj_data, double t){
    ...
    double pen;
    // The wheel-alpha estimate is numerically sensitive near zero velocity.
    // A 5% tolerance rejected otherwise trackable seeds and forced four full
    // replans; retain a bounded 50% engineering tolerance for this numerical
    // wheel-alpha estimate.
    double feas_tol_percent_ = 0.50;
    ...
    if(pen > 0.0){
      ROS_WARN_THROTTLE(1.0, "%f max left wheel omega exceeds the soft limit at relative time %f", wheel_omega_left, t );
    }
    ...
    if(pen > 0.0){
      ROS_WARN_THROTTLE(1.0, "%f max right wheel omega exceeds the soft limit at relative time %f", wheel_omega_right, t );
    }
    ...
    if(pen > 0.0){
      ROS_WARN_THROTTLE(1.0, "%f max left wheel alpha exceeds the soft limit at relative time %f", wheel_alpha_left, t );
    }
    ...
    if(pen > 0.0){
      ROS_WARN_THROTTLE(1.0, "%f max right wheel alpha exceeds the soft limit at relative time %f", wheel_alpha_right, t );
    }
  ...
  bool PolyTrajOptimizer::obstacleGradCostforMM(const int i_dp,
    ...
    // Use a smooth URDF-mesh clearance cost and numerical gradient.  Unlike
    // a boolean collision predicate, this provides a direction for the arm
    // to move while the base advances toward the goal.
    if (mm_config_->usesUrdfCollisionMesh()) {
      const Eigen::VectorXd q = pos;
