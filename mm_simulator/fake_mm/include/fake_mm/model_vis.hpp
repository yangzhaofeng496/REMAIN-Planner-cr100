...
namespace model_vis{
    ...
    private:
        ...
        bool use_robot_description_tf_;
        bool reset_actual_path_;
        ...
        void planningStartCallback(const std_msgs::Bool::ConstPtr& state);
        void trajectoryCallback(const quadrotor_msgs::PolynomialTraj::ConstPtr& msg);
        void plannedEePathCallback(const nav_msgs::Path::ConstPtr& msg);
        void tfTimerCallback(const ros::TimerEvent & event);
        void vis_his_traj(Eigen::Vector2d pt);
        void publishActualEePath();
        ros::Subscriber joint_state_sub_, odom_sub_, gripper_state_sub_, planning_start_sub_, trajectory_sub_, planned_ee_path_sub_;
        ros::Publisher vis_mm_pub_, vis_mm_check_ball_pub_, vis_his_traj_pub_, ee_path_pub_;
        nav_msgs::Path ee_path_;
        nav_msgs::Path planned_ee_path_;
