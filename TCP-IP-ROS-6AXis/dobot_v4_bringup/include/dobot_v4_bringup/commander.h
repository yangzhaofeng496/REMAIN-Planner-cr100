...
public:
    ...
    {
        uint32_t has_read;
        RealTimeData temp_real_time_data;
        while (is_running_) {
            // ROS_INFO("recvTask variables: is_running_ : %d, real_time_tcp_ : %d", is_running_.load(), real_time_tcp_->isConnect());
            if (real_time_tcp_->isConnect()) {
                try {
                    if (real_time_tcp_->tcpRecv(&temp_real_time_data, sizeof(temp_real_time_data), has_read, 5000)) {
                        // ROS_INFO("real_time_q_actual : %f, %f, %f, %f, %f, %f", temp_real_time_data.q_actual[0], temp_real_time_data.q_actual[1],
                        //          temp_real_time_data.q_actual[2], temp_real_time_data.q_actual[3], temp_real_time_data.q_actual[4],
                        //          temp_real_time_data.q_actual[5]);
                        // ROS_INFO("temp_real_time_data.len : %d", temp_real_time_data.len);
                        if (temp_real_time_data.len != 1440)
                            continue;

                        mutex_.lock();
                        real_time_data_ = temp_real_time_data;
...
private:
    ...
    static void doTcpCmd_f(std::shared_ptr<TcpClient>& tcp, const char* cmd, int32_t& err_id,
                           std::string& mode_id, std::vector<std::string>& result)
    {
        try {
            uint32_t has_read;
            char buf[1024];
            ...
            std::size_t start_pos = str_buf.find('{');
            std::size_t end_pos = str_buf.find('}');
            if (start_pos != std::string::npos && end_pos != std::string::npos && end_pos > start_pos) {
                mode_id = str_buf.substr(start_pos + 1, end_pos - start_pos - 1);
            } else {
                mode_id = "";
            }
