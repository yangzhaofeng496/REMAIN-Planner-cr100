		...
		case POLY_TRAJ:
		{
			...
			|| trajectory_data.exec_traj != 1 || trajectory_data.singul_traj_data.singul_traj.empty()){
				if(trajectory_data.exec_traj != -1){
					...
					// Hold the endpoint evaluated from the same full trajectory used
					// while executing.  getJuncPos() may return a stale junction cache
					// after deserialising a multi-piece PolynomialTraj, which made the
					// arm jump back to the zero configuration at trajectory completion.
					const double duration = trajectory_data.singul_traj_data.duration;
					Eigen::VectorXd end_pos = trajectory_data.singul_traj_data.getPos(duration);
					Eigen::VectorXd end_vel = trajectory_data.singul_traj_data.getVel(duration);
					int singul = trajectory_data.singul_traj_data.getSingul(duration);
					stay_joint_ = end_pos.tail(manipulator_dof_);
					stay_pos_ = end_pos.head(mobile_base_dof_);
					ROS_WARN_STREAM("[MMctrl] trajectory end duration=" << duration
						<< " end_pos=" << end_pos.transpose());
					Eigen::Vector2d stay_vel_ = end_vel.head(mobile_base_dof_);
...
void MMControllerFSM::cal_traj_ctrl_input(double duration){
	...
	// Controller-side diagnostic: compare the state used by the simulator or
	// hardware feedback with the exact polynomial sample being commanded.
	// This separates execution lag from TF/FK display errors.
	if (state_data.have_car_ && state_data.have_mani_) {
		const double car_err = (cmd_data.car_true_cmd.head(2) - state_data.car_p.head(2)).norm();
		const double joint_err = (cmd_data.joint_pos_cmd - state_data.joint_p).norm();
		ROS_WARN_THROTTLE(1.0, "[MMCtrl] tracking state error t=%.3f car=%.4f joint=%.4f",
		                  duration, car_err, joint_err);
	}
}
